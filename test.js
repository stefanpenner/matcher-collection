...
tap.Test.prototype.addAssert('mayContain', 2, function (path, matcher) {
    this.assert(new MatcherCollection(matcher).mayContain(path), 'expected: `' + path + '` to match: `' + matcher + '`');
});
tap.Test.prototype.addAssert('mayNotContain', 2, function (path, matcher) {
    this.assert(!new MatcherCollection(matcher).mayContain(path), 'expected to NOT: `' + path + '` to match: `' + matcher + '`');
});
